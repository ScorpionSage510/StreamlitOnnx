...
# --- Ton code OCR ici, je simplifie et conserve la fonction run_ocr() ---

# (Ta fonction run_ocr reste inchangée, ici juste un exemple minimal)
def run_ocr(image_pil):
    # Simuler une détection (à remplacer par ton vrai OCR)
    # Retourne des boxes en dict
    width, height = image_pil.size
    return [
        {"box": [[10, 10], [width//2, height//2]], "text": "Demo", "confidence": 0.9}
    ]

# --- Streamlit API simulée ---

params = st.experimental_get_query_params()

if "img_url" in params:
    # Mode API : on télécharge l'image et retourne JSON
    ...
    try:
        ...
        st.error(f"Erreur lors du chargement ou OCR : {e}")

else:
    # Mode interface utilisateur classique
...
#         boxes_positions.append({"top_left": pt1, "bottom_right": pt2})

#     return boxes_positions

# # --- Streamlit UI ---

# st.title("Détection de boîtes OCR (positions uniquement)")
...
#     if not boxes:
#         st.write("Aucune boîte détectée.")
#     else:
...
#         st.image(image, caption="Boîtes détectées", use_column_width=True)
        
#         st.write("Positions des rectangles détectés :")
#         st.json(boxes)
# # import streamlit as st 
# # from PIL import Image, ImageDraw
# # import numpy as np
...
# #     # On ne calcule plus le texte, on renvoie uniquement les positions (boîtes)
# #     boxes_positions = []
# #     for box in boxes:
# #         (x1, y1), (x2, y2) = box
# #         pt1 = (int(x1 * scale_x), int(y1 * scale_y))
...
# #             st.write("Aucune boîte détectée.")
# #         else:
# #             # Dessiner uniquement les boîtes détectées
# #             draw = ImageDraw.Draw(image)
# #             for box in boxes:
# #                 pt1 = box["top_left"]
# #                 pt2 = box["bottom_right"]
